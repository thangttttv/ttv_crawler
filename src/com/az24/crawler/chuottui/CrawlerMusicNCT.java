		...
		//	String link_singer = (String) reader.read(xpath__tag+"["+ i + "]/p[1]/a[1]/@href", XPathConstants.STRING);
			...
			for (String string : m_singer_arr) {
				...
				if(temp_singer_id ==0&&!StringUtil.isEmpty(m_singer))
				{
					...
					//this.extractSinger("http://www.nhaccuatui.com"+link_singer);				
...
//		String image = (String) reader.read(xpath_image, XPathConstants.STRING);
	...
	public static void main(String[] args) {
		CrawlerMusicNCT crawlerMusicZing = new CrawlerMusicNCT();
		MusicSource.initSourceLinkNCT();
		...
		try {
			...
			for (MusicSource musicSource : mList) {
				...
				{		
				 ...
				 crawlerMusicZing.collectionLink(musicSource.url+"&page="+i, musicSource.parent_cat_id, musicSource.cat_id, musicSource.language,type_zing,"Việt Nam", i);
			 	 i++;
				}
			}
		} catch (Exception e) {
