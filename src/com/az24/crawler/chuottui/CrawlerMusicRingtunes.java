...
public class CrawlerMusicRingtunes {
	...
	public void collectionLink(String url,int cate_parent_id,int category_id, int language,int type_zing,String nationality, int page) throws Exception {
		if(url.indexOf("http://")==-1) url = "http://ringtunes.vinaphone.com.vn/"+url;
		...
		String xpath__tag= "//TABLE[@id='table7']/TBODY[1]/TR";
		...
		while (i <= node_one_many) {
			String m_title = (String) reader.read(xpath__tag+"["+ i + "]/TD[2]/SPAN[1]", XPathConstants.STRING);
			String m_singer = (String) reader.read(xpath__tag+"["+ i + "]/TD[2]/SPAN[2]", XPathConstants.STRING);
			String m_code_mback = (String) reader.read(xpath__tag+"["+ i + "]/TD[1]", XPathConstants.STRING);
	...
	public static void main(String[] args) {
		CrawlerMusicRingtunes crawlerMusicZing = new CrawlerMusicRingtunes();
		MusicSource.initSourceLinkRingtunes();
		...
		try {
			...
			for (MusicSource musicSource : mList) {
				...
				{		
				 ...
				 crawlerMusicZing.collectionLink(musicSource.url+i, musicSource.parent_cat_id, musicSource.cat_id, musicSource.language,type_zing,"Việt Nam", i);
			 	 i++;
				}
			}
		} catch (Exception e) {
