...
public class ProductTempDAO {
	...
	public int updateImageMain(int product_id, String picture) {
		...
		try {
			...
			ps = connection
					.prepareStatement(" Update products set picture = ? where id = ? ");
	...
	public int deletePricture(int product_id) {
		...
		try {
			...
			connection.setAutoCommit(false);
			ps = connection
					.prepareStatement(" DELETE from products_picture where product_id = ? ");
	...
	public int getProductByName(String name, int cat_id) {
		...
		try {
			Connection connection = C3p0StorePool.getConnection();
			ps = connection
					.prepareStatement(" SELECT id  FROM products WHERE trim(name) = ? And cat_id = ? ");
	...
	public int saveProduct(Product product) {
		...
		try {
			...
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(
					" INSERT INTO products_temp (cat_id,cat_id1,cat_id2,cat_id3,cat_id4,cat_id5,name,picture,"
							+ "  is_multi,status,price_from,price_to,create_user,create_date, modify_user,modify_date,original_link" +
								 ",url_rewrite,description)"
							+ "  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	public int updateName(Product product) {
		...
		try {
			...
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(
					" update  products set name = ?, url_rewrite = ? where id = ?");
	...
	public boolean savePicture(ProductImage image) {
		...
		try {
			...
			strSQL = new StringBuffer(
					" INSERT INTO products_picture_temp  (product_id,name, path, description, kbyte, "
